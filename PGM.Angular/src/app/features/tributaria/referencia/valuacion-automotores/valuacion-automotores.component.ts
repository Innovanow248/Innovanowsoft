...
<div class="page-container">
  ...
  <div class="card filter-card">
    <mat-form-field appearance="outline" class="field-ano">
      <mat-label>Año valuación</mat-label>
      <select matNativeControl [ngModel]="anoSel" (ngModelChange)="anoSel=$event; cargar()">
    ...
    <mat-form-field appearance="outline" class="field-busq">
      <mat-label>Filtrar por CIP</mat-label>
      <mat-icon matPrefix>search</mat-icon>
      <input matInput [(ngModel)]="filtroCip" (ngModelChange)="filtrar()" placeholder="Ej: 0010103" />
    </mat-form-field>
    <div class="filter-info">{{ filasFiltradas.length | number }} registros</div>
  </div>
    ...
    <div class="card table-card">
      <table mat-table [dataSource]="pagina">
        ...
        <ng-container matColumnDef="modeloValuacion">
          ...
          <td mat-cell *matCellDef="let v">{{ v.modeloValuacion }}</td>
        </ng-container>
  ...
  styles: [`
    ...
    .filter-card{padding:16px 20px;display:flex;align-items:center;gap:16px;margin-bottom:16px;flex-wrap:wrap}
    .field-ano{width:160px} .field-busq{flex:1;min-width:200px}
...
export class ValuacionAutomotoresComponent {
  ...
  page     = 0;
  pageSize = 100;
  cols = ['anoValuacion','cip','modeloValuacion','baseImponible','alicuota','accion'];
  ...
  constructor() {
    this.svc.anosValuacion().subscribe({ next: d => { this.anos = d; if (d.length) { this.anoSel = d[0]; this.cargar(); } } });
  }
  ...
  cargar() {
    ...
    this.filtroCip = '';
    this.page = 0;
    this.svc.valuacionAutomotores(this.anoSel).subscribe({
      next: d => { this.todos = d; this.filtrar(); this.loading.set(false); },
  ...
  filtrar() {
    ...
    this.filasFiltradas = f ? this.todos.filter(v => v.cip.toLowerCase().includes(f)) : [...this.todos];
    this.page = 0;
  }
